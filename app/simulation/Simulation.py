...
class Simulation(object):
    ...
    def processRemovals(cls, delta):
        ...
        for removedCarId in removedList:
            ...
            CarRegistry.findById(removedCarId).setArrived(
                cls.tick, despawn)
        ...
        if p > 1:
            ...
            for _ in range(delta_to_nuke):
                ...
                CarRegistry.findById(removedCarId).setArrived(
                    cls.tick, despawn=True)

    @classmethod
    def updateSimulation(cls):
    ...
    @classmethod
    def processAdditions(cls, delta):
        if delta < 0:
            ...
            if Config.kafkaUpdates is False and Config.mqttUpdates is False:
                ...
                if newConf is not None:
                    ...
                    if "edge_average_influence" in newConf:
                        ...
                        print("setting edgeAverageInfluence: " +
                              str(newConf["edge_average_influence"]))
    ...
    def loop(cls):
        ...
        P, I, D = 1 / (0.2 * BASE_DELAY), 1 / \
            (0.2 * BASE_DELAY), 1 / (0.2 * BASE_DELAY)
        pid = PID(P, I, D, normalize=True)
        def remapper(u): return int(round(u))
        ...
        while 1:
            ...
            cls.kafkaTopicPerformance()

            delta = cls.pidUpdate(errorHistory, pid, remapper)
            cls.processRemovals(delta)
            cls.updateSimulation()
            cls.processAdditions(delta)

            cls.pullKafka()
            cls.printStatus()
            cls.pushKafka()

            cls.carCountSanityCheck()